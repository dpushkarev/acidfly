...
if ($mode == "post") {
    ...
    if ($mode == "Submit") {
        if (!$name OR !$comments)
            ...
            $ckresult = mysqli_query($dblink, $ckquery) or die ("Unable to check. Try again later.");
            if (mysqli_num_rows($ckresult) == 0) {
...
// Set page numbers
        ...
        $gbresult = mysqli_query($dblink, $gbquery) or die ("Unable to select totals. Please try again later.");
        $totalgbresult = mysqli_query($dblink, $totalgbquery) or die ("Unable to access total records.");
        $totalgbnum = mysqli_num_rows($totalgbresult);
...
// END DISPLAY OF PAGE NUMBERS
            ...
            for ($gb = 1; $gbrow = mysqli_fetch_row($gbresult); ++$gb) {
