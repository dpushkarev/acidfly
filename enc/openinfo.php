...
if ($wspass AND $wsemail) {
    ...
    $memresult = mysqli_query($dblink, $memquery) or die ("Unable to select. Try again later.");
    if (mysqli_num_rows($memresult) == 1) {
        $memrow = mysqli_fetch_array($memresult);
...
$colorresult = mysqli_query($dblink, $colorquery) or die ("Unable to select your system variables. Try again later.");
...
// If customer selects category, find records with that category
    if ($category) {
        ...
        $parentresult = mysqli_query($dblink, $parentquery) or die ("Unable to select category. Try again later.");
        ...
        if (mysqli_num_rows($parentresult) > 0) {
            $maincat_set = "yes";
            for ($p = 1; $parentrow = mysqli_fetch_row($parentresult); ++$p) {
...
// START MULTI CATEGORY
                ...
                $endresult = mysqli_query($dblink, $endquery) or die ("Unable to select category. Try again later.");
                if (mysqli_num_rows($endresult) > 0) {
                    for ($s = 1; $endrow = mysqli_fetch_row($endresult); ++$s) {
...
// END MULTI CATEGORY
        ...
        $metaresult = mysqli_query($dblink, $metaquery) or die ("Unable to select category. Try again later.");
        if (mysqli_num_rows($metaresult) == 1) {
            $metarow = mysqli_fetch_array($metaresult);
            if ($metarow[Keywords])
...
// If customer chooses an item, find that item (for customizes)
    if ($item) {
        ...
        $metaresult = mysqli_query($dblink, $metaquery) or die ("Unable to select item. Try again later.");
        if (mysqli_num_rows($metaresult) == 1) {
            $metarow = mysqli_fetch_array($metaresult);
            if ($metarow[Keywords])
                $Meta_Keywords = stripslashes($metarow[Keywords]);
...
$pgnum = mysqli_num_rows($pgresult);
if ($pgnum == 1) {
