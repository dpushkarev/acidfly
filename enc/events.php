...
if ($event) {
    ...
    $desresult = mysqli_query($dblink, $desquery) or die ("Unable to select. Try again later.");
    while ($desrow = mysqli_fetch_row($desresult)) {
    ...
    $desresult = mysqli_query($dblink, $desquery) or die ("Unable to select. Try again later.");
    if (mysqli_num_rows($desresult) > 0) {
