...
$delresult = mysql_query($delquery, $dblink) or die ("Unable to select. Try again later.");
while ($delrow = mysql_fetch_row($delresult))
{
$itemid = $delrow[0];
...
$catsresult = mysql_query($catsquery, $dblink) or die ("Unable to select. Try again later.");
$catsnum = mysql_num_rows($catsresult);
if ($catsnum == 0)
...
$itemsresult = mysql_query($itemsquery, $dblink) or die ("Unable to select. Try again later.");
$itemsnum = mysql_num_rows($itemsresult);
if ($itemsnum == 0)
...
$defresult = mysql_query($defquery, $dblink) or die ("Unable to select. Try again later.");
$defrow = mysql_fetch_row($defresult);
}
...
$selresult = mysql_query($selquery, $dblink) or die ("Unable to select. Try again later.");
$selnum = mysql_num_rows($selresult);
if ($selnum == 1 AND !empty($fields[0]))
{
$selrow = mysql_fetch_row($selresult);
$selid = $selrow[0];
$setquery = "UPDATE " .$DB_Prefix ."_items SET $updatelines WHERE ID='$selid'";
$setresult = mysql_query($setquery, $dblink) or die ("Unable to upload. Try again later.");
}
else
{
$setquery = "INSERT INTO " .$DB_Prefix ."_items ($insertfields) VALUES ($insertlines)";
$setresult = mysql_query($setquery, $dblink) or die ("Unable to upload. Try again later.");
$selid = mysql_insert_id();
}
...
$deloresult = mysql_query($deloquery, $dblink) or die("Unable to delete. Please try again later.");
$deliquery = "DELETE FROM " .$DB_Prefix ."_inventory WHERE ProductID='$selid'";
$deliresult = mysql_query($deliquery, $dblink) or die("Unable to delete. Please try again later.");
...
while ($getrow = mysql_fetch_row($getresult))
{
...
<table border=0 cellpadding=3 cellspacing=0 class="generaltable">
<tr>
<td valign="middle" align="center" class="fieldname" colspan="2">Global Sale Price Updates</td>
</tr>
<tr>
<td valign="middle" align="center" colspan="2">Update sale prices for all items in a category:</td>
</tr>
...
$getcat_1result = mysql_query($getcat_1query, $dblink) or die("Could not select categories");
for ($getcat_1count = 1; $getcat_1row = mysql_fetch_row($getcat_1result); ++$getcat_1count)
{
...
$subcat_1result = mysql_query($subcat_1query, $dblink) or die ("Could not show categories. Try again later.");
$subcat_1num = mysql_num_rows($subcat_1result);
...
{
while ($subcat_1row = mysql_fetch_row($subcat_1result))
{
...
$endcat_1result = mysql_query($endcat_1query, $dblink) or die ("Could not show categories. Try again later.");
// No there are no endcats so display the parent and subcat only
if (mysql_num_rows($endcat_1result) == 0)
echo "<option value=\"$subcat_1row[1]\">$ss_stripcategory_1 &gt; $stripsubcat_1</option>";
else
{
while ($endcat_1row = mysql_fetch_row($endcat_1result))
{
...
<td valign="middle" align="left">
<input type="radio" value="percent" checked name="type"> percent OFF the regular price<br>
<input type="radio" value="amount" name="type"> <?php echo "$currency"; ?> amount OFF the regular price<br>
<input type="radio" value="exact" name="type"> exact <?php echo "$currency"; ?> amount<br>
...
<tr>
<td valign="middle" align="center" colspan="2">Delete all items in the following category:</td>
</tr>
<tr>
<td valign="middle" align="right">Select Category:</td>
<td valign="middle" align="left">
...
$getcat_presult = mysql_query($getcat_pquery, $dblink) or die("Could not select categories");
for ($getcat_pcount = 1; $getcat_prow = mysql_fetch_row($getcat_presult); ++$getcat_pcount)
{
// Are there subcategories?
$subcat_pquery = "SELECT Category, ID FROM " .$DB_Prefix ."_categories WHERE Parent = '$getcat_prow[1]' ORDER BY CatOrder, Category";
$subcat_presult = mysql_query($subcat_pquery, $dblink) or die ("Could not show categories. Try again later.");
$subcat_pnum = mysql_num_rows($subcat_presult);
...
{
while ($subcat_prow = mysql_fetch_row($subcat_presult))
{
...
$endcat_presult = mysql_query($endcat_pquery, $dblink) or die ("Could not show categories. Try again later.");
// No there are no endcats so display the parent and subcat only
if (mysql_num_rows($endcat_presult) == 0)
echo "<option value=\"$subcat_prow[1]\">$ss_stripcategory_p &gt; $stripsubcat_p</option>";
else
{
while ($endcat_prow = mysql_fetch_row($endcat_presult))
{
...
<tr>
<td valign="middle" align="center">
Upload a tab delimited text file of items into your database. Use the <a href="files/sample.txt" target="_blank">sample sheet</a>  as a guide to create your own text
file, and keep all columns as-is. This process can completely modify your current product listing, so create a system back up first!</td>
</tr>
<tr>
<td valign="middle" align="center">
