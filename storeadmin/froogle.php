...
$parcatresult = mysql_query($parcatquery, $dblink) or die ("Unable to select. Try again later.");
if (mysql_num_rows($parcatresult) == 1)
{
$parcatrow = mysql_fetch_row($parcatresult);
if ($parcatrow[1] > 0)
{
$gpcatquery = "SELECT Category FROM " .$DB_Prefix ."_categories WHERE ID='$parcatrow[1]'";
$gpcatresult = mysql_query($gpcatquery, $dblink) or die ("Unable to select. Try again later.");
if (mysql_num_rows($gpcatresult) == 1)
$gpcatrow = mysql_fetch_row($gpcatresult);
}
}
}
}
...
if (mysql_num_rows($chkresult) == 1 AND $_POST[splitoptions] == "Yes")
{
...
$invresult = mysql_query($invquery, $dblink) or die ("Unable to select options. Try again later.");
$invchk = mysql_num_rows($invresult);
...
$fvarresult = mysql_query($fvarquery, $dblink) or die ("Unable to select. Try again later.");
$fvarrow = mysql_fetch_row($fvarresult);
...
you with this tool to help you load your products easily.</td>
</tr>
<tr>
<td align="right" nowrap>Froogle Server:</td>
<td colspan="3"><input type="text" name="froogleserver" value="<?php echo "$froogle_server"; ?>" size="30"> 
&nbsp; &nbsp; &nbsp;<input type="checkbox" name="deletefile" value="Yes" checked>Delete File</td>
</tr>
<tr>
<td align="right" nowrap>User Name:</td>
<td align="left"><input type="text" name="froogleuser" value="<?php echo "$froogle_user"; ?>" size="10"></td>
<td align="right" nowrap>Password:</td>
<td align="left"><input type="password" name="frooglepass" value="<?php echo "$froogle_pass"; ?>" size="10"></td>
</tr>
<tr>
<td align="right" nowrap>ISO Currency:</td>
<td><input type="text" name="froogleconversion" value="<?php echo "$froogle_conv"; ?>" size="10" maxlength="3"></td>
