...
if ($Submit == "Add Vendor") {
    ...
    if ($company) {
        ...
        $getwsresult = mysqli_query($dblink, $getwsquery) or die("Unable to check your vendor. Please try again later.");
        if (mysqli_num_rows($getwsresult) == 0) {
            ...
            $wsresult = mysqli_query($dblink, $wsquery) or die("Unable to edit your vendor. Please try again later.");
        } else
...
if ($Submit == "Yes, Delete Vendor") {
    ...
    $delwsresult = mysqli_query($dblink, $delwsquery) or die("Unable to delete this item. Please try again later.");
}

if ($Submit == "Add" OR $Submit == "Edit") {
    if ($Submit == "Edit" AND $wsid) {
        $getwsquery = "SELECT * FROM " . $DB_Prefix . "_wholesale WHERE ID = '$wsid'";
        $getwsresult = mysqli_query($dblink, $getwsquery) or die ("Could not show categories. Try again later.");
        $getwsrow = mysqli_fetch_array($getwsresult);
                ...
                <table border=0 cellpadding=0 cellspacing=0 class="generaltable">
                    ...
                    <tr>
                        ...
                        <td valign="top" align="left">
                            <input type="text" name="password" value="<?php echo "$ws_pswd"; ?>" size="10"
                                   maxlength="10">
                        </td>
    ...
    $getwsresult = mysqli_query($dblink, $getwsquery) or die ("Could not show categories. Try again later.");
    $getwsrow = mysqli_fetch_row($getwsresult);
                ...
                <table border=0 cellpadding=0 cellspacing=0 class="generaltable">
                    <tr>
                        <td>
                            ...
                            <input type="submit" class="button" value="Yes, Delete Vendor" name="Submit">&nbsp; <input
    ...
    <div align="center">
        <center>
            <table border=0 cellpadding=3 cellspacing=0 class="largetable">
                ...
                $wsresult = mysqli_query($dblink, $wsquery) or die ("Could not show categories. Try again later.");
                while ($wsrow = mysqli_fetch_array($wsresult)) {
                    ...
                    $ordresult = mysqli_query($dblink, $ordquery) or die ("Could not show vendors. Try again later.");
                    ...
                    <table border=0 cellpadding=3 cellspacing=0 class="generaltable">
                        <tr>
                            <td valign="middle" align="center">
                                ...
                                $getwsresult = mysqli_query($dblink, $getwsquery) or die ("Could not show categories. Try again later.");
                                $getwsnum = mysqli_num_rows($getwsresult);

                                if ($getwsnum == 0)
                                    echo "No Vendors Listed.";
                                else {
                                    echo "<select size=\"1\" name=\"wsid\">";
                                    for ($getwscount = 1; $getwsrow = mysqli_fetch_row($getwsresult); ++$getwscount) {
            ...
            <div align="center">
                <center>
                    <table border=0 cellpadding=3 cellspacing=0 class="generaltable">
                        <tr>
                            <td valign="middle" align="center">
                                The wholesale page is currently inactive. If you would like to<br>activate it, please
