...
$updatecresult = mysql_query($updatecquery, $dblink) or die("Unable to update colors. Please try again later.");
}
if ($updateimages == "yes")
{
...
$updatevresult = mysql_query($updatevquery, $dblink) or die("Unable to update colors. Please try again later.");
}
}
}

$thmquery = "SELECT ThemeName FROM " .$DB_Prefix ."_vars WHERE ID='1'";
$thmresult = mysql_query($thmquery, $dblink) or die ("Unable to select. Try again later.");
$thmrow = mysql_fetch_row($thmresult);
$theme_name = $thmrow[0];
...
<tr>
<td align="center">
Choose a template style below or add a custom theme to your site:</td>
</tr>
<tr>
<td align="center">
...
<td align="left">
<blockquote>
<input type="checkbox" name="updatecolors" checked value="yes"> Update Link Colors (overrides current color settings)
<br><input type="checkbox" name="updateimages" value="yes"> Update Logo/Images (overrides current image settings)
</blockquote>
</td>
</tr>
<tr>
