...
if ($Submit == "Edit Guest") {
    ...
    if ($name) {
        ...
        $updresult = mysqli_query($dblink, $updquery) or die("Unable to edit your guest. Please try again later.");
    }
}

if ($Submit == "Yes, Delete Guest") {
    $delgbquery = "DELETE FROM " . $DB_Prefix . "_guestbook WHERE ID = '$gbid'";
    $delgbresult = mysqli_query($dblink, $delgbquery) or die("Unable to delete this item. Please try again later.");
}
...
if ($mode == "Add" OR $mode == "Edit") {
    if ($mode == "Edit") {
        ...
        $getgbresult = mysqli_query($dblink, $getgbquery) or die ("Could not show categories. Try again later.");
        $getgbrow = mysqli_fetch_row($getgbresult);
                ...
                <table border=0 cellpadding=0 cellspacing=0 class="generaltable">
                    <tr>
                        <td>
                            ...
                            echo "<input type=\"hidden\" value=\"$gbid\" name=\"gbid\">";
                            ?>
                            <input type="submit" class="button" value="Yes, Delete Guest" name="Submit">&nbsp; <input
    ...
    if ($setactive == "Yes") {
        ...
        $getaresult = mysqli_query($dblink, $getaquery) or die ("Could not show categories. Try again later.");
        $getanum = mysqli_num_rows($getaresult);
        if ($getanum > 0) {
            ?>
            <form method="POST" action="guestbook.php">
                <div align="center">
                        ...
                        <table border=0 cellpadding=0 cellspacing=0 class="generaltable">
                            <tr>
                                <td valign="middle" align="center">
                                    ...
                                    for ($getacount = 1; $getarow = mysqli_fetch_row($getaresult); ++$getacount) {
                    ...
                    <table border=0 cellpadding=0 cellspacing=0 class="generaltable">
                        <tr>
                            <td valign="middle" align="center">
                                ...
                                $getgbresult = mysqli_query($dblink, $getgbquery) or die ("Could not show categories. Try again later.");
                                $getgbnum = mysqli_num_rows($getgbresult);

                                if ($getgbnum == 0)
                                    echo "<i>No Guestbook Entries Listed.</i>";
                                else {
                                    echo "<select size=\"1\" name=\"gbid\">";
                                    for ($getgbcount = 1; $getgbrow = mysqli_fetch_row($getgbresult); ++$getgbcount) {
            ...
            <div align="center">
                <center>
                    <table border=0 cellpadding=3 cellspacing=0 class="generaltable">
                        <tr>
                            <td valign="middle" align="center">
                                The guestbook page is currently inactive. If you would like to<br>activate it, please
