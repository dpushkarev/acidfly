...
// DELETE ALL OPTION INVENTORY
            ...
            $delattresult = mysqli_query($dblink, $delattquery);
...
// UPDATE MAIN ITEM INVENTORY
        ...
        $updattresult = mysqli_query($dblink, $updattquery);
    }
// END PRODUCT LOOP
}
...
if ($catsearch) {
    ...
    $catsearchresult = mysqli_query($dblink, $catsearchquery) or die ("Unable to select. Try again later.");
    if (mysqli_num_rows($catsearchresult) == 1) {
        $catsearchrow = mysqli_fetch_row($catsearchresult);
    ...
    if (mysqli_num_rows($subresult) > 0) {
        while ($subrow = mysqli_fetch_row($subresult)) {
            ...
            $endresult = mysqli_query($dblink, $endquery) or die ("Unable to select. Try again later.");
            if (mysqli_num_rows($endresult) > 0) {
                while ($endrow = mysqli_fetch_row($endresult)) {
...
$itemresult = mysqli_query($dblink, $itemquery) or die ("Unable to view items.");
$itemnum = mysqli_num_rows($itemresult);

// Total records altogether
$totalitemresult = mysqli_query($dblink, $totalitemquery) or die ("Unable to view items.");
$totalitemnum = mysqli_num_rows($totalitemresult);
        ...
        <form action="itemlist.php" method="POST">
            <div align="center">
                <table class="generaltable" cellSpacing="0" cellPadding="0" border="0">
                    ...
                    <?php
                    for ($i = 1; $itemrow = mysqli_fetch_array($itemresult); ++$i) {
...
// CHECK INVENTORY FOR EACH ITEM
                        ...
                        $getinvresult = mysqli_query($dblink, $getinvquery) or die("Could not select options.");

// SINGLE ITEM INVENTORY
                        if (mysqli_num_rows($getinvresult) != 1) {
                            echo "<tr";
                            if ($i % 2 == 0)
                            ...
                            $getinvrow = mysqli_fetch_array($getinvresult);
...
// Check for inv quantities
                                ...
                                $invresult = mysqli_query($dblink, $invquery) or die("Could not select inv qtys.");
                                $invnum = mysqli_num_rows($invresult);
                                if ($invnum == 1) {
                                    $invrow = mysqli_fetch_row($invresult);
                                    $inventory = $invrow[0];
                                } else
                                    $inventory = "";
                                if ($v == 0) {
        ...
        <form action="itemlist.php" method="POST" name="EditForm">
            <div align="center">
                <center>
                    <table class="generaltable" cellSpacing="0" cellPadding="0" border="0">
                        <?php
                        for ($i = 1; $itemrow = mysqli_fetch_row($itemresult); ++$i) {
                            $prodid = $itemrow[0];
                            $catnum = $itemrow[1];
...
// No there are no endcats so display the parent and subcat only
                                ...
                                <td vAlign="top" align="right" colSpan="2" class="smalltext">
                                    ...
                                    if (mysqli_num_rows($optresult) == 1) {
