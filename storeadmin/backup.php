...
if ($submit == "Create Database Backup") {
    ...
    $tableresult = mysqli_query($dblink, $tablequery) or die ("Unable to select. Try again later.");
    $dateformat = date("l, F j, Y g:i a", mktime(0, 0, 0, date("m"), date("d"), date("Y")));
    $snquery = "SELECT SiteName FROM " . $DB_Prefix . "_vars WHERE ID='1'";
    $snresult = mysqli_query($dblink, $snquery) or die ("Unable to select. Try again later.");
    $snrow = mysqli_fetch_row($snresult);
    $sitename = $snrow[0];
...
// Go through records and create insert tables
            ...
            $recresult = mysqli_query($dblink, $recquery) or die ("Unable to select. Try again later.");
            while ($recrow = mysqli_fetch_row($recresult)) {
