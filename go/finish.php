...
$chkresult = mysql_query($chkquery, $dblink) or die ("Unable to select order. Try again later.");
...
$checkqtyresult = mysql_query($checkqtyquery, $dblink) or die ("Unable to select attribute inventory. Try again later.");
$checkqtynum = mysql_num_rows($checkqtyresult);
if ($checkqtynum == 1)
{
$checkqtyrow = mysql_fetch_row($checkqtyresult);
$newinvqty = $checkqtyrow[0] - $quantity;
if ($newinvqty < 0)
$newinvqty = 0;
$updqtyquery = "UPDATE " .$DB_Prefix ."_inventory SET Quantity='$newinvqty' WHERE ID='$checkqtyrow[1]'";
$updqtyresult = mysql_query($updqtyquery, $dblink) or die("Unable to update. Please try again later.");
$countquery = "SELECT SUM(Quantity) FROM " .$DB_Prefix ."_inventory WHERE ProductID='$itemid'";
$countresult = mysql_query($countquery, $dblink) or die("Unable to count. Please try again later.");
$countrow = mysql_fetch_row($countresult);
$updinvquery = "UPDATE " .$DB_Prefix ."_items SET Inventory='$countrow[0]' WHERE ID='$itemid'";
$updinvresult = mysql_query($updinvquery, $dblink) or die("Unable to update. Please try again later.");
if ($newinvqty == 0)
...
$checkinvresult = mysql_query($checkinvquery, $dblink) or die ("Unable to check item inventory. Try again later.");
$checkinvnum = mysql_num_rows($checkinvresult);
if (mysql_num_rows($checkinvresult) == 1)
{
$checkinvrow = mysql_fetch_row($checkinvresult);
...
$inssalesresult = mysql_query($inssalesquery, $dblink) or die("Unable to add. Please try again later.");
